...
def flatten(itemList):
    ...
    for item in itemList:
        rtn = rtn | (flatten(item) if ___________________________________
    return rtn
...
def charm(itGroups, minSup, cSets):
   for idx1, grp1 in enumerate(itGroups):
      ...
      newSet = ________________________
...
def main():
   ...
   itmSets.sort(key=_____________________) 
...
def main():
    ...
    # means.shape =  _____________     
    # sigmas.shape = _______________  
    # prbs.shape = _______________    
