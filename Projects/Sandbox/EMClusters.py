...
def main():
    ...
    sigmas = np.ones((numCls, dim)) if diagonal else 
     np.full((numCls, dim, dim), np.identity(dim))
    ...
    while meansDelta > eps:     # while (adjust > eps):
        ...
        for clsIdx in range(0, numCls):
            ...
            if diagonal:
                invSigma = np.reciprocal(sigmas[clsNdx]) * -.5
...
def report(pts, ptDiffs, weights, means, sigmas, prbs, meansDelta, dbgPts):
    ...
    for mIdx, mean in enumerate(means):
        ...
        plt.gca().add_patch(Ellipse(xy=mean, width = 2*eVals[1],
            height = 2*eVals[0], facecolor='None', edgecolor=color, linewidth=2,
